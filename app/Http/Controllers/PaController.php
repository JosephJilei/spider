...
class PaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //@$html->load_file($url);
        @$html->load_file(storage_path().'/app/public/html/48-'.$item.'.html');
    ...
    private function httpCurl($url, $header=null)
    {
        ...
        curl_setopt($ch,CURLOPT_TIMEOUT,0);//超时时间(必须)
    ...
    private function export($data, $name, $group, $item) 
    {
        ...
        Excel::store(new ProductExport($data), $name.'-'.$group.'-'.$item.'.xlsx', 'public');
    }
